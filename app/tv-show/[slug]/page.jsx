...
import {
  ...
  getTvSeriesByGenre,
  getTvSeriesGenres,
  fetchMediaItem, // ✅ Tambahkan ini
  generateSlug, // ✅ Tambahkan ini
  checkForDuplicateTitleYear, // ✅ Tambahkan ini
} from '../../../lib/api';
...
export async function generateMetadata({ params }) {
  ...
  let tvShowData = null;
  try {
    tvShowData = await fetchMediaItem(slug, 'tv');
  } catch (error) {
    console.error('Error fetching TV show data for metadata:', error);
  }

  if (!tvShowData) {
    return { title: 'Sflix - TV Show Not Found' };
  }
  ...
  return {
    ...
    openGraph: {
      ...
      description: tvShowData.overview,
      url: `https://sflix-watch.vercel.app/tv-show/${slug}`,
      siteName: 'Sflix',
...
export default async function TvShowPage({ params }) {
  ...
  if (genreSlugMap.has(processedSlug)) {
    ...
    // ✅ Check duplicates untuk genre series
    if (series && series.length > 0) {
      await checkForDuplicateTitleYear(series);
    }
  ...
  if (CATEGORIES.includes(slug)) {
    ...
    // ✅ Check duplicates untuk kategori series
    if (series && series.length > 0) {
      await checkForDuplicateTitleYear(series);
    }
  ...
  // ✅ Check duplicates untuk similar TV series
  if (similarTvSeries && similarTvSeries.length > 0) {
    await checkForDuplicateTitleYear(similarTvSeries);
  }
